...
def plot_particle_field_heatmap(result_df, filename="particle_field_heatmap.png"):
    ...
    # ✅ รองรับสองชื่อคอลัมน์
    if "Ψ(n)" not in df.columns and "psi" in df.columns:
        df = df.rename(columns={"psi": "Ψ(n)"})

    if "Ψ(n)" not in df.columns:
        raise ValueError("result_df ต้องมีคอลัมน์ 'Ψ(n)' หรือ 'psi'")

    plt.figure()
    sns.heatmap(df.set_index("number")[["Ψ(n)"]].T, cbar=True)  # จะกำหนด cmap ก็ได้ถ้าคุณต้องการ
